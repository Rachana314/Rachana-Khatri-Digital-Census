    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="w-full max-w-lg rounded-3xl bg-white border shadow-xl p-6">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onClose} disabled={busy} className="rounded-2xl px-5 py-2 font-black border hover:bg-zinc-50 disabled:opacity-50">
            Cancel
          </button>
  ...
  return (
    ...
    >
      <div className="relative max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
        <button
          onClick={onClose}
          className="absolute -top-12 right-0 text-white font-black text-lg px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 transition"
        >
          ✕ Close
        </button>
        <img
        ...
        />
        {name && (
          <div className="text-white/70 text-center font-bold text-sm mt-3">{name}</div>
        )}
      </div>
    </div>
  );
...
export default function HouseholdView() {
  ...
  if (loading)
    return <div className="p-20 text-center font-black text-zinc-400 text-xl">Loading record...</div>;
  if (err)
    return <div className="p-20 text-center font-black text-rose-500">Error: {err}</div>;
  if (!item)
    return <div className="p-20 text-center font-black text-rose-500">Household record not found.</div>;

  // Group server-side documents by memberName
  const docsByMember = {};
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          to="/admin/households"
          className="font-black text-zinc-400 hover:text-zinc-900 transition"
        >
          ← Back to List
        </Link>
      ...
      <div className="bg-white border rounded-3xl p-8 shadow-sm">
        ...
        <div className="space-y-8">
          {item.members?.map((m) => {
            ...
            // Combine member.photo + linked docs into one photos array
            const allPhotos = [];
            if (photoUrl) {
              allPhotos.push({
                url: photoUrl,
                label: m.docType || "Document Photo",
              });
            }
                  ...
                  <div>
                    ...
                    {m.citizenshipId && (
                      <div className="text-zinc-500 text-xs font-mono mt-1">
                        Citizenship ID:{" "}
                        <span className="font-black text-zinc-800 tracking-wider">
                          {m.citizenshipId}
                        </span>
                      </div>
                    )}
                  ...
                  {allPhotos.length === 0 ? (
                    ...
                    <div
                      className={`grid gap-4 ${
                        allPhotos.length === 1 ? "grid-cols-1" : "grid-cols-2"
                      }`}
                    >
                      {allPhotos.map((photo, i) => (
                        ...
                        >
                          ...
                          />
                          {/* PDF / non-image fallback */}
                          <div
                            className="w-full items-center justify-center bg-zinc-100 text-zinc-500 font-bold text-sm flex-col gap-2"
                            style={{ display: "none", minHeight: "220px" }}
                          >
                          ...
                          {/* Zoom hint */}
                          <div className="absolute top-3 right-3 bg-black/50 text-white text-[10px] font-bold px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                            🔍 Enlarge
                          </div>
                        </div>
                      ))}
